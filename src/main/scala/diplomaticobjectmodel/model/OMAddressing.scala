...
object OMRegister {
  ...
  private def makeRegisterMap(rawModule: RawModule,
    baseAddress: BigInt,
    mapping: Seq[(Int, Seq[RegField])]): Option[OMRegisterMap] = {
  ...
  private def getPerms(): OMPermissions = {
    OMPermissions(
      readable = false,
      writeable = false,
      executable = false,
  ...
  private def makeMemoryRegionFromRegisters(
    ...
    OMMemoryRegion (
      name = "",
      description = "",
      addressSets = Nil, //Seq[OMAddressSet],
      permissions =  getPerms(),
      registerMap = makeRegisterMap(rawModule, baseAddress, mapping)
    )
  }
  ...
  def convert(
    ...
    makeMemoryRegionFromRegisters(rawModule, baseAddress, mapping)
  }
